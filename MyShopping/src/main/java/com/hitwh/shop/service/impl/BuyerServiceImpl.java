...
//            UserSku currentShoppingCartColumn = shoppingCartRepository.findUserSkuBySkuIdAndUserId(skuId, userId);
            if(currentSku == null){//检查这个单品是否存在
                result.construct(false, ConstUtils.MSG_ADD_FAILED,null);
            }else{//单品存在
    ...
    public void deleteShoppingCart(Integer skuId,HttpSession session,BaseResult<Object> result){
        ...
        userSkuRepository.deleteUserSkuBySkuIdAndUserId(skuId,userId);
//         Address address= addressRepository.findAddressByUserId(userId);
//        orderSkuRepository.findbyOrderId(orderId);
////        OrderSku orderSku = new OrderSku(order.getId(),skuId,amount); //order_sku
////        orderSkuRepository.deleteById(orderId);
//        orderSkuRepository.deleteOrderSkuById(orderId);
//        orderSkuRepository.flush();
...
//        orderRepository.deleteOrderById(orderId);
    ...
    public void saveUserInfo(BaseResult<Object> result, String headImg, String provinceName, String cityName, String countyName, String detail, String phone, String password, HttpSession session) {
        ...
        if (addressRepository.findAddressByUserId(userId) == null){
            ...
            String CountryId = county.getCountyId();
            Address address = new Address(userId, ProvinceId, CityId, CountryId, detail, phone, 0, 0);
    ...
    public void createStore(String description, HttpSession session, BaseResult<Object> result) {
        ...
        // 更新Session中的权限
        List<PermissionResult> permissionResultList = roleRepository.getPermissionByUserId(userId);
    ...
    public void addOrder(BaseResult<Object> result, Integer spuId, Integer skuId, Integer amount, String spuName, BigDecimal price,  HttpSession session){
        Integer userId = (Integer) session.getAttribute(ConstUtils.SESSION_USER_ID);
        Address address= addressRepository.findAddressByUserId(userId);
        Order order = new Order(userId,address.getId(),new Timestamp(System.currentTimeMillis())); //r_order
        orderRepository.saveAndFlush(order);
        OrderSku orderSku = new OrderSku(order.getId(),skuId,amount); //order_sku
        orderSkuRepository.saveAndFlush(orderSku);
        result.construct(true, ConstUtils.MSG_SUCCESS, order);
//        r_OrderRepository.save(rOrderResult);
//        OrderSkuResult orderSku = new OrderSkuResult(skuId,amount,1) ;
