...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @Query(value = "select new com.hitwh.shop.model.pojo.OrderInfoResult(o.id, sp.name, os.number, o.date) "+
            "from Order o, Sku sk, Spu sp, OrderSku os "+
            "where o.id= :orderId and os.orderId = o.id and sk.id = os.skuId and sp.id = sk.spuId ")
    OrderInfoResult findOrderInfoById(Integer orderId);

    @Transactional
    @Modifying
    @Query(value = "update Order o set o.state = 1 where o.id = :orderId")
