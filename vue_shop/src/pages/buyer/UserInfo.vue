          ...
          <div>
            <template><div><el-form label-width="80px" class="info_form">
              ...
              <el-form-item label="用户密码">
                <el-input v-model="password" id="password" type="password" disabled="true"></el-input>
              </el-form-item>
            ...
            <template><div style="text-align: center;margin-top: 30px">我的店铺</div>
              <div><el-form label-width="80px" class="store_form">
                <el-form-item label="商铺名称" style="margin-top: 30px">
                  <el-input id="store" v-bind:disabled="storeFlag" v-model="storeName"></el-input>
                </el-form-item>
                <el-form-item class="btns" style="margin-top: 30px">
                  <el-button type="primary" @click="button1" v-bind:disabled="editFlag">编辑</el-button>
                  <el-button id="button2" type="primary" @click="button2">保存</el-button>
                </el-form-item>
...
export default {
  ...
  methods: {
    editFlagCheck () {
      this.$axios({
        methods: 'get',
        url: 'myshopping/buyer/getUserRole'
      }).then(response => {
        if (response.status !== 200) {
          ...
          if (response.data.success === true) {
            if (response.data.data === 1) {
              ...
              this.storeName = response.data.data.description
              this.editFlag = true
              this.roleId = 2
              document.getElementById('button2').innerText = '进入店铺'
            }
          }
        }
      })
      ...
      this.$axios({
        ...
        if (response.status !== 200) {
          ...
          if (response.data.success === true) {
            ...
            this.imageUrl = response.data.data.headImg
            console.log(response.data.data.headImg)
    ...
    button2 () {
      if (this.roleId === 1) {
        this.$axios({
          methods: 'get',
          url: 'myshopping/buyer/createStore',
          params: { description: this.storeName }
        }).then(response => {
          if (response.status !== 200) {
    ...
    save () {
      ...
      this.$axios({
        methods: 'get',
        url: 'myshopping/buyer/saveUserInfo',
        params: { headImg: this.imageUrl, province: this.province, city: this.city, county: this.county, detail: this.details, phone: this.phone, password: this.password }
      }).then(response => {
        if (response.status !== 200) {
          this.$message.error('保存失败！')
          return this.$message.error('登录失败！')
