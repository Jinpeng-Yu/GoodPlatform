...
<!--      table表格区-->
      <el-table :data="goodslist" border stripe>
        ...
        <el-table-column label="商品描述" prop="description" width="300px">
          <template slot-scope="scope">
            <div v-html="scope.row.description" class="html-box"></div>
          </template>
        </el-table-column>
...
<!--          <el-input v-model="editForm.img"></el-input>-->
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editSpuInfo">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    editSpuInfo () {
      ...
      this.$refs.editFormRef.validate(valid => {
        ...
        this.$axios({
          methods: 'get',
          url: 'myshopping/seller/editSpuInfo',
          params: {
            spuId: this.editForm.id,
            spuName: this.editForm.name,
            spuImg: this.imageUrl,
            spuIntroduce: this.editForm.description
          }
        }).then(response => {
          if (response.status !== 200) {
